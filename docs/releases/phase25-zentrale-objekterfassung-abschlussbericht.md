...
1. ~~**Kein einzelnes Mega-Formular**~~ — **KORRIGIERT in Phase 26** (siehe `docs/releases/phase26-zentrale-objekterfassung-korrektur-ein-formular.md`): Kevin hat diesen Punkt in einem verbindlichen Korrekturauftrag als nicht ausreichend zurückgewiesen. Es existiert jetzt genau ein sichtbarer Objekteditor (`#kkCentralObjectEditorDialog`/`#kkcrmproObjectForm`) für alle drei Typen, kein Router mehr zwischen drei separaten Formularen.
